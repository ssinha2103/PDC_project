...
//  mpi_project
//
//  Created by Kishan Kumar Sharma on 7/10/21.
//  Copyright © 2021 Kishan Kumar Sharma. All rights reserved.
//
...
int Read_n(int my_rank, MPI_Comm comm); MPI_Datatype Build_blk_col_type(int n, int loc_n); void Read_matrix(int loc_mat[], int n, int loc_n,MPI_Datatype blk_col_mpi_t, int my_rank, MPI_Comm comm);
...
MPI_Type_free(&block_mpi_t); MPI_Type_free(&first_bc_mpi_t);
...
void Print_matrix(int loc_mat[], int n, int loc_n,MPI_Datatype blk_col_mpi_t, int my_rank, MPI_Comm comm) { int* mat = NULL, i, j;
